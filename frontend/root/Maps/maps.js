...
const { getAllDiscounts , getAllStores } = require('../functions/Getters.js');
...
async function populateSubcategories(subcategorySelect) {
  const response = await fetch(`${config.backend.url}:${config.backend.port}/getSubcategories`);
...
async function onMarkerClick(marker,e,id){
  try {
    const response = await fetch(`${config.backend.url}:${config.backend.port}/getDiscountedItems?shopId=${id}`);
