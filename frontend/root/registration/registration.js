...
document.getElementById('registrationForm').addEventListener('submit', async function(event) {
  ...
  try {
    ...
    const response = await fetch(`${config.backend.url}:${config.backend.port}/register`, {
      method: 'POST',
      headers: {
...
document.getElementById('loginForm').addEventListener('submit', async function(event) {
  ...
  try {
    ...
    const response = await fetch(`${config.backend.url}:${config.backend.port}/login`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
document.getElementById('a').addEventListener('click', async function(event) {
  event.preventDefault();
  fetch(`${config.backend.url}:${config.backend.port}/get-session-data`, {
    method: 'GET',
